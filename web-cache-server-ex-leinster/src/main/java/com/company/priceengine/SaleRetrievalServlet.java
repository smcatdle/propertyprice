...
public class SaleRetrievalServlet extends HttpServlet {
	...
	public synchronized void initialize() {
		...
		Coordinate gridTopLeft = null;
		Coordinate gridBottomRight = null;
		Map<String, GraphPoint> graphPoints = null;
		GridWrapper gridWrapper = null;
		List<PropertyMDO> properties = null;
		...
		try {
			...
			// Get all the grids outside the peak ViewPort
			List<Integer> gridIds = PropertyDAO.getInstance()
					.getGridsOutsideViewPort(peakViewPort.getTopLeftCoord(), peakViewPort.getBottomRightCoord());

			logger.log(Level.INFO, "Retrieved [" + gridIds.size() + "] gridIds...");
			
			// Iterate through all the grids within the peak ViewPort
			for (int gridId : gridIds) {

				logger.log(Level.INFO, "Retrieved Grid  [" + gridId + "]");

				// Initialise the Grid ViewPort
				gridViewPort = viewPortGridDivisor.getCellViewPort(gridId);
				gridTopLeft = new Coordinate();
				gridBottomRight = new Coordinate();
				gridTopLeft.setLat(gridViewPort.getTopLeftCoord().getLat());
				gridTopLeft.setLng(gridViewPort.getTopLeftCoord().getLng());
				gridBottomRight.setLat(gridViewPort.getBottomRightCoord().getLat());
				gridBottomRight.setLng(gridViewPort.getBottomRightCoord().getLng());
				
				properties = getPropertiesWithinGrid(gridId);

				if (properties != null && properties.size() > 0) {

					// Calculate the averages and quantity for each year
					graphPoints = FastPropertyRetrieveDAO.getInstance()
							.retrieveGridGraphData(gridViewPort);

					gridWrapper = new GridWrapper();
					gridWrapper.setG(graphPoints);
					gridWrapper.setP(properties);

					key = gridViewPort.getTopLeftCoord().getLat() + "-"
							+ gridViewPort.getTopLeftCoord().getLng() + "-"
							+ gridViewPort.getBottomRightCoord().getLat() + "-"
							+ gridViewPort.getBottomRightCoord().getLng();
	...
	public List<PropertyMDO> getPropertiesWithinGrid(int gridId) {
		...
		try {
			properties = PropertyDAO.getInstance()
					.findPropertiesInGrid(gridId);

			if (properties.size() > 0)
				logger.log(Level.INFO, "getPropertiesWithinGrid size ["
						+ properties.size() + "]");
